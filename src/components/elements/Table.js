...
const calculateVisiblePages = (currentPage, numPages, maxVisiblePages = 7) => {
  ...
  if (currentPage - offset <= 1) {
    const startingPages = [...Array(maxVisiblePages - 2).keys()].map(
      (index) => index + 1
    );
  ...
  return (
    <div className="flex-1 flex items-end justify-end gap-4">
      {visiblePageNumbers.map((pageNum, idx) => {
        ...
        const isNotConsecutive =
          idx > 0 && visiblePageNumbers?.[idx - 1] !== pageNum - 1;
        return (
          <>
...
export const LegacyPageSelection = ({
  currentPage,
  numPages,
  onPageSelect,
  disabled,
        ...
        return (
          <button
            ...
            className={cx("px-2 py-0.5 outline-1", {
              ...
              "hover:outline hover:text-blue-700 hover:outline-blue-400":
                !isCurrentPage,
              "focus:outline focus:text-blue-700 focus:outline-blue-400":
                !isCurrentPage,
            })}
            key={idx}
...
export const SortableHeader = ({
  ...
  const handleClick = () =>
    onSelect(field, isSelected ? getOppositeSort(sortDir) : "dsc");
  return (
    <HeaderCell>
      <div
        className={cx(
      ...
      >
        {children}
        {isSelected && (
          <FontAwesomeIcon icon={sortDir === "asc" ? faCaretUp : faCaretDown} />
        )}
      </div>
    </HeaderCell>
  );
